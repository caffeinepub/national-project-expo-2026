...
// ─── Backend Patch: Remove admin-only auth guards ─────────────────────────────
// The admin dashboard uses a frontend password (not Internet Identity).
// Backend write functions must be open to all callers so the password-gated
// admin panel can actually save changes.
async function patchBackend() {
  const accessControlPath = "src/backend/authorization/access-control.mo";
  try {
    let content = await fs.readFile(accessControlPath, "utf-8");
    // Make isAdmin always return true so frontend-password-gated calls succeed
    content = content.replace(
      /public func isAdmin\(state : AccessControlState, caller : Principal\) : Bool \{\s*getUserRole\(state, caller\) == #admin;\s*\};/,
      "public func isAdmin(state : AccessControlState, caller : Principal) : Bool {\n    // Access is gated by the frontend password — all callers treated as admin.\n    true;\n  };"
    );
    // Make hasPermission always return true
    content = content.replace(
      /public func hasPermission\(state : AccessControlState, caller : Principal, requiredRole : UserRole\) : Bool \{[\s\S]*?\};/,
      "public func hasPermission(state : AccessControlState, caller : Principal, requiredRole : UserRole) : Bool {\n    true;\n  };"
    );
    await fs.writeFile(accessControlPath, content, "utf-8");
    console.log("[patch] Backend access-control patched: all callers treated as admin.");
  } catch (err) {
    console.warn("[patch] Could not patch access-control.mo:", err.message);
  }
}
