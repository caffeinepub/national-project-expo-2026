...
// Build a lookup map: stageName -> desc, from static data
const STAGE_DESC_MAP: Record<string, string> = Object.fromEntries(
  STATIC_TIMELINE.map((s) => [s.stageName, s.desc]),
);

function getDescForStage(stageName: string): string {
  return STAGE_DESC_MAP[stageName] ?? "";
}

export default function TimelinePage() {
  const { data: backendTimeline, isLoading } = useAllTimelineStages();

  const rawStages =
    backendTimeline && backendTimeline.length > 0
      ? backendTimeline.map((s) => ({
  ...
  return (
    <section id="timeline" className="py-24 relative pt-28">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 relative z-10">
        ...
        {isLoading ? (
          <div
            data-ocid="timeline.loading_state"
            className="flex items-center justify-center py-20"
          >
            <div className="flex flex-col items-center gap-3">
              <CalendarClock className="w-8 h-8 text-primary animate-pulse" />
          ...
          >
            <div className="w-16 h-16 rounded-2xl bg-accent/10 border border-accent/25 flex items-center justify-center mb-4">
              <CalendarClock className="w-8 h-8 text-accent/50" />
            </div>
            <p className="text-foreground font-display font-black text-lg mb-1">
              Timeline coming soon
            </p>
            <p className="text-muted-foreground text-sm max-w-xs">
              Important dates and milestones will be announced here. Check back
              soon.
            </p>
          </div>
        ) : (
